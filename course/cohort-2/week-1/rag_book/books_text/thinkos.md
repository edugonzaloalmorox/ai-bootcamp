...
The L [A] TEX source for this book is available from `[http://greenteapress.com/](http://greenteapress.com/thinkos)`
...
goals is to help them learn C. For that part of the class, I use Griffiths and Griffiths, _Head First C_, from O’Reilly Media. This book is meant to complement
that one.
...
reading: _Head First C_ Chapter 2.
...
works. Recommended reading: _Head First C_ Chapter 6.
...
Chapter 9 is about POSIX threads and mutexes. Recommended reading: _Head_
_First C_ Chapter 12 and _Little Book of Semaphores_ Chapters 1 and 2.


Chapter 10 is about POSIX condition variables and the producer/consumer
problem. Recommended reading: _Little Book of Semaphores_ Chapters 3 and
4.
...
Example code for this book is available from `[https://github.com/](https://github.com/AllenDowney/ThinkOS)`
`[AllenDowney/ThinkOS](https://github.com/AllenDowney/ThinkOS)` . Git is a version control system that allows you to
...
People often describe programming languages as either compiled or interpreted. “Compiled” means that programs are translated into machine language and then executed by hardware; “interpreted” means that programs
...
So being compiled or interpreted is not an intrinsic characteristic of a language; nevertheless, there are some general differences between compiled and
...
by looking at the program what type each variable refers to. In a dynamicallytyped language, you don’t always know the type of a variable until the program is running. In general, **static** refers to things that happen at compile
...
Declaring types at compile time also saves space. In dynamic languages, variable names are stored in memory while the program runs, and they are often accessible by the program. For example, in Python the built-in function
...
By default, `gcc` stores the executable code in a file called `a.out` (which originally stood for “assembler output”). The second line runs the executable. The
...
Similar to the `-c` flag, the `-S` flag tells `gcc` to compile the program and generate
...
Now that we know the steps in the compilation process, it is easier to understand error messages. For example, if there is an error in a `#include` directive,
...
click on a link, the browser displays the page the link refers to. The software and network communication that make that possible are complex,
...
For example, many public libraries participate in inter-library collaborations that allow them to borrow books from each other. When I request
...
One of the most important goals of an operating system is to isolate each running program from the others so that programmers don’t have to think about
...
you have _b_ bits, you can indicate one of 2 _[b]_ values. A **byte** is 8 bits, so it can
...
In general, if you want to specify one of _N_ values, you should choose the
smallest value of _b_ so that 2 _[b]_ _≥_ _N_ . Taking the log base 2 of both sides yields
_b ≥_ _log_ 2 _N_ .
...
_log_ 26 bits of information. And in general, if the probability of the outcome is
1 in _N_, then the outcome contains _log_ 2 _N_ bits of information.


Equivalently, if the probability of the outcome is _p_, then the information content is _−log_ 2 _p_ . This quantity is called the **self-information** of the outcome.
...
and forth between the number of bits, _b_, and the number of values they can
encode, _N_ = 2 _[b]_ .
...
is usually some kind of random access memory (RAM). On most current computers, main memory is **volatile**, which means that when the computer shuts
down, the contents of main memory are lost. A typical desktop computer has
2–8 GiB of memory. GiB stands for “gibibyte,” which is 2 [30] bytes.
...
“gigabyte,” which is 10 [9] bytes. TB stands for “terabyte,” which is 10 [12] bytes.
...
runs from 0 to _N −_ 1, where _N_ is the size of main memory. On a system
with 1 GiB of physical memory, the highest valid address is 2 [30] _−_ 1, which is
1,073,741,823 in decimal, or 0x3f f in hexadecimal (the prefix 0x indicates
...
to _M −_ 1, where _M_ is the number of valid virtual addresses. The size of the
...
the virtual address space runs from 0 to 0xf f . The size of this address
space is 2 [32] bytes, or 4 GiB.


On a 64-bit system, the size of the virtual address space is 2 [64] bytes, or 2 [4] _·_ 1024 [6]
...
When a program reads and writes values in memory, it generates virtual addresses. The hardware, with help from the operating system, translates to
...
If you add this function to `aspace.c` you can confirm that `counter` is allocated
...
 Since 1 GiB is 2 [30] bytes and 1 KiB is 2 [10] bytes, there are 2 [20] physical
pages, sometimes called “frames.”


 The size of the virtual address space is 2 [32] B and the size of a page is
2 [10] B, so there are 2 [22] virtual pages.


 The size of the offset is determined by the page size. In this example the
page size is 2 [10] B, so it takes 10 bits to specify a byte on a page.
...
 Since there are 2 [20] physical pages, each physical page number is 20 bits.
...
frame. So we expect 3–4 bytes per entry. But with 2 [22] virtual pages, the page
table would require 2 [24] bytes, or 16 MiB.


And since we need a page table for each process, a system running 256 processes
would need 2 [32] bytes, or 4 GiB, just for page tables! And that’s just with 32-bit
...
includes both the virtual page number and the physical page number. Searching an associative table can be slow in software, but in hardware we can search
the entire table in parallel, so associative arrays are often used to represent
the page table entries in the TLB.


You can read more about these implementations at `[http://en.wikipedia.](http://en.wikipedia.org/wiki/Page_table)`
`[org/wiki/Page_table](http://en.wikipedia.org/wiki/Page_table)` ; you might find the details interesting. But the fundamental idea is that page tables are sparse, so we have to choose a good
...
is a kind of key-value database (see `[https://en.wikipedia.org/wiki/](https://en.wikipedia.org/wiki/Key-value_database)`
`[Key-value_database](https://en.wikipedia.org/wiki/Key-value_database)` ).
...
**24** **Chapter 4.** **Files and fle systems**
...
ms (see `[https://en.wikipedia.org/wiki/Hard_disk_drive_performance_](https://en.wikipedia.org/wiki/Hard_disk_drive_performance_characteristics)`
`[characteristics](https://en.wikipedia.org/wiki/Hard_disk_drive_performance_characteristics)` ). SSDs are faster, taking 25 _µ_ s to read a 4 KiB block and
250 _µ_ s to write one (see `[http://en.wikipedia.org/wiki/Ssd#Controller](http://en.wikipedia.org/wiki/Ssd#Controller)` ).
...
**26** **Chapter 4.** **Files and fle systems**


In one microsecond, it would complete 2000 instructions, so while waiting 25
_µ_ s for a byte from an SSD, it would complete 50,000.
...
a linked list. For more details, see `[http://en.wikipedia.org/wiki/File_](http://en.wikipedia.org/wiki/File_Allocation_Table)`
`[Allocation_Table](http://en.wikipedia.org/wiki/File_Allocation_Table)` .


**28** **Chapter 4.** **Files and fle systems**
...
One example is the UNIX pipe, which is a simple form of inter-process communication. Processes can be set up so that output from one process is taken
...
**30** **Chapter 4.** **Files and fle systems**
...
for example, the representation for 510 is _b_ 101.


For negative numbers, the most obvious representation uses a sign bit to indicate whether a number is positive or negative. But there is another representation, called “two’s complement” that is much more common because it
is easier to work with in hardware.


To find the two’s complement of a negative number, _−x_, find the binary representation of _x_, flip all the bits, and add 1. For example, to represent _−_ 510,
start with the representation of 510, which is _b_ 00000101 if we write the 8-bit
version. Flipping all the bits and adding 1 yields _b_ 11111011.
...
**Clearing bits** : For any value _x_, _x_ &0 is 0, and _x_ &1 is _x_ . So if you AND a
...
**5.3.** **Representing foating-point numbers** **33**
...
**Setting bits** : Similarly, for any _x_, _x|_ 0 is x, and _x|_ 1 is 1. So if you OR a vector
...
m/s is approximately 2 _._ 998 _·_ 10 [8] .
...
In the 32-bit standard, the leftmost bit is the sign bit, _s_ . The next 8 bits
are the exponent, _q_, and the last 23 bits are the coefficient, _c_ . The value of a
floating-point number is
( _−_ 1) _[s]_ _c ·_ 2 _[q]_


Well, that’s almost correct, but there’s one more wrinkle. Floating-point numbers are usually normalized so that there is one digit before the point. For
example, in base 10, we prefer 2 _._ 998 _·_ 10 [8] rather than 2998 _·_ 10 [5] or any other
...
For example, the integer representation of 1310 is _b_ 1101. In floating point,
that’s 1 _._ 101 _·_ 2 [3], so the exponent is 3 and the part of the coefficient that would
...
standard. See `[http://en.wikipedia.org/wiki/IEEE_floating_point](http://en.wikipedia.org/wiki/IEEE_floating_point)` .
...
Also, the letters _and numbers_ in C strings are encoded in ASCII. The ASCII
codes for the digits “0” through “9” are 48 through 57, _not_ 0 through 9. The
...
is why most modern languages provide higher-level memory management features like garbage collection.

## **6.1 Memory errors**


The C memory management API is a bit like Jasper Beardly, a minor character on the animated television program _The Simpsons_ ; in a few episodes, he
...
 If you read a value from an unallocated chunk, the system _might_ detect
...
with C-style memory management is that the data structures used to implement `malloc` and `free` (which we will see soon) are often stored along with the
allocated chunks. So if you accidentally write past the end of a dynamicallyallocated chunk, you are likely to mangle these data structures. The system
...
memory, you should also provide an interface to free it, and memory management should be part of the API design from the beginning.
...
elements of the implementation is available at `[http://gee.cs.oswego.edu/](http://gee.cs.oswego.edu/dl/html/malloc.html)`
`[dl/html/malloc.html](http://gee.cs.oswego.edu/dl/html/malloc.html)` .
...
The cache “hit rate”, _h_, is the fraction of memory accesses that find data in
cache; the “miss rate”, _m_, is the fraction of memory accesses that have to go
to memory. If the time to process a cache hit is _Th_ and the time for a cache
miss is _Tm_, the average time for each memory access is
...
a cache miss, _Tp_ = _Tm −_ _Th_ . Then the average access time is


_Th_ + _mTp_


When the miss rate is low, the average access time can be close to _Th_ . That
...
|8<br>(ns)<br>6<br>penalty<br>average<br>4<br>2<br>0<br>2|512 B<br>256 B<br>128 B<br>64 B<br>32 B<br>16 B<br>8 B|Col3|Col4|Col5|
|---|---|---|---|---|
|2<br>0<br>2<br>4<br>6<br>8<br>average penalty (ns)|12<br>2<br>14<br>|2<br>|16<br>2<br>|18<br>2<br>20<br>2<br>22<br>2<br>24<br>2<br>26|
...
iteration, it _subtracts_ the elapsed time from `sec` . When the loop completes,
...
the cache size _or_ if the stride is smaller than the block size. Performance only
degrades if the array is bigger than the cache _and_ the stride is large.


In Figure 7.1, cache performance is good, for all strides, as long as the array
is less than 2 [22] B. We can infer that the cache size is near 4 MiB; in fact,
...
a little when the array size is bigger than 2 [14] B, so it’s possible that this
...
they break big arrays into smaller pieces and then work with the pieces. Sometimes these algorithms can be tuned to take advantage of cache behavior.


For applications where performance is critical, it is possible to design algorithms tailored to the size of the cache, the block size, and other hardware
...
Without going too far into computer architecture, there are two reasons: electronics and economics. Caches are fast because they are small and close to
...
|Device|Access<br>time|Typical<br>size|Cost|
|---|---|---|---|
|Register|0.5 ns|256 B|?|
|Cache|1 ns|2 MiB|?|
|DRAM|100 ns|4 GiB|$10 / GiB|
|SSD|10_ µ_s|100 GiB|$1 / GiB|
...
not been used recently. For more on this topic, see `[http://en.wikipedia.](http://en.wikipedia.org/wiki/Cache_algorithms)`
`[org/wiki/Cache_algorithms](http://en.wikipedia.org/wiki/Cache_algorithms)` .
...
might try to maximize throughput, which is the number of requests that complete per unit of time.
...
controls motors might have to complete recurring tasks at some minimum frequency and react to external events with some maximum response time. These
...
For more demanding applications, especially in domains where real-time response is a matter of life and death, “real-time operating systems” provide
...
This compiles a source file named `array.c` with debugging info and optimization, links with the Pthread library, and generates an executable named
`array` .
...
In this case, the only shared variable is `counter` . `make` ~~`s`~~ `hared` allocates space
...
typecast it accordingly and then pass it along to `child` ~~`c`~~ `ode`, which does the
real work.
...
`pthread` `join` . Here is my wrapper for `pthread` ~~`j`~~ `oin` :
...
wrapper does is call `pthread` ~~`j`~~ `oin` and check the result.
...
a circular buffer, which you can read about at `[https://en.wikipedia.org/](https://en.wikipedia.org/wiki/Circular_buffer)`
`[wiki/Circular_buffer](https://en.wikipedia.org/wiki/Circular_buffer)` .
...
If there are threads waiting on the condition variable, one of them gets unblocked and resumes execution of `cond_wait` . But before the awakened thread
...
true. If you have reason to think the condition _might_ be true, you could call
...
`make` ~~`s`~~ `emaphore` takes the initial value of the semaphore as a parameter.
It allocates space for a Semaphore, initializes it, and returns a pointer to
`Semaphore` .


`sem` ~~`i`~~ `nit` returns 0 if it succeeds and -1 if anything goes wrong. One nice thing
...
Here’s the new definition of `Queue`, replacing the mutex and condition variables
with semaphores:
...
`items` is the number of items in the queue, which is also the number of consumer threads that can execute `queue_pop` without blocking. Initially there
...
the semaphore keeps track of how many spaces are available and blocks producers if the queue is full.
...
This solution is explained, using pseudo-code, in Chapter 4 of _The Little Book_
_of Semaphores_ .
...
Before you go on, you might want to try this as an exercise: write functions that implement the semaphore API in `sem.h` using using condition variables and mutexes. In the repository for this book, you’ll find my solution in
...
With the `do...while` loop, it is guaranteed [1] that when a thread signals, one
...
1Well, almost. It turns out that a well-timed spurious wakeup (see `[http://en.](http://en.wikipedia.org/wiki/Spurious_wakeup)`
`[wikipedia.org/wiki/Spurious_wakeup](http://en.wikipedia.org/wiki/Spurious_wakeup)` ) can violate this guarantee.
